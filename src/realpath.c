/* realpath.c -- prints resolved absolute pathname.
   Copyright © 2017-2024 David Winter
...
enum { RELATIVE_TO_OPTION, RELATIVE_BASE_OPTION, HELP_OPTION, VERSION_OPTION };
...
void warn_ignored(char c) {
  ...
  if (opt->val == c) {
    warnx("the option \"--%s\" is not supported.", opt->name);
  } else {
    warnx("the option \"-%c\" is not supported.", c);
  }
}
...
int main(int argc, char **argv) {
  ...
  while ((ch = getopt_long(argc, argv, "emhLPqsz", longopts, &longindex)) !=
         -1) {
    switch (ch) {
    case 'm':
    case 'L':
